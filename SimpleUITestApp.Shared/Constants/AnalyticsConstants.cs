namespace SimpleUITestApp.Shared
{
	public static class AnalyticsConstants
	{
		public const string MOBILE_CENTER_iOS_API_KEY = "36f454de-dbe0-4a38-a11b-68323a21a52e";
		public const string GO_BUTTON_TAPPED = "Go Button Tapped";
		public const string TEXT_ENTERED = "Text Entered";
		public const string FIRST_PAGE_ON_APPEARING = "First Page";
		public const string LIST_VIEW_PAGE_ON_APPEARING = "List View Page";
		public const string LIST_VIEW_ITEM_TAPPED = "List View Item Tapped";
		public const string LIST_VIEW_ITEM_NUMBER = "List View Item Number";
		public const string ACTIVITY_INDICATOR_ONSCREEN = "Activity Indicator On Screen";
	}
}

